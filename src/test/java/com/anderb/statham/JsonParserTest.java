...
class JsonParserTest {
    ...
    void findEndElementIndex() {
        String json = "{\n" +
                ...
                "  }\n" +
                "}";
        assertEquals(63, JsonParser.findEndingElementIndex(json, 1, 66, '{', '}'));
    }

    @Test
    void skipWhiteSpaces() {
        assertEquals(0, JsonParser.skipWhiteSpaces("", 0, 0));
        assertEquals(2, JsonParser.skipWhiteSpaces("  ", 0, 1));
        assertEquals(1, JsonParser.skipWhiteSpaces(" 0", 0, 1));
        assertEquals(3, JsonParser.skipWhiteSpaces(":  [", 1, 3));
    }
    ...
    void countBetween() {
        ...
        assertEquals(5, JsonParser.countBetween("[][][][][][][][][]", 0, 9, ']'));
        assertEquals(0, JsonParser.countBetween("{{{{{{{{{{", 0, 9, '}'));
        assertEquals(9, JsonParser.countBetween("{{{{{{{{{{", 0, 9, '{'));
        assertEquals(8, JsonParser.countBetween("{{{{{{{{{}", 0, 9, '{'));
    }

    @Test
    void parseToList() {
        var json = "[1,2,3,4]";
        List actual = (List) JsonParser.parseToList(json, 0, json.length() - 1).getValue();
